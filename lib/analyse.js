...
OrderResult.prototype = {
    ...
    endParse: function() {
        this.defer.resolve({
            ...
            coverage: this.coverage / Object.keys(this.browsers).length
        })
    },
    ...
    when.all(promises, function() {
       ...
       // console.info(str)
        Object.keys(LogResult.all).sort().forEach(function(date) {
          parseResult(date, LogResult.all[date])
        })
    })
    //console.info('logs', logs)
}

function parseResult(date, orders) {
    console.info('======' + date + '=======')
    var overview = {
        succ: 0,
        failure: 0,
    ...
    orderIds.forEach(function(orderId){
        ...
        var test = result[repo] || (result[repo] = {num: 0, succ: 0, failure: 0, time: 0, coverage: 0})
        test.num++
        if (orderInfo.succ) {
            test.succ++
    ...
    Object.keys(result).forEach(function(repo) {
        ...
        info.averageTime =  info.time / info.succ
        info.coverage = info.coverage / info.succ
        overview.succ = overview.succ + info.succ
        overview.failure = overview.failure + info.failure

        if (info.succ) {
            overview.coverage = overview.coverage + (info.coverage * info.succ)
        }
    })

    overview.coverage = overview.coverage / overview.succ
