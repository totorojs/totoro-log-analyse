...
function MongoLog(models) {
  ...
  this.repoInfoUpdateQueue = async.queue(function(log, callback) {
    ...
    var RepoInfoModel = models.RepoInfoModel
...
MongoLog.prototype = {
  appendLog: function(log) {
    var that = this
    var LogModel = this.models['LogModel']
...
module.exports = function() {
  ...
  return {
    analyse: function(models) {
      log = new MongoLog(models)
      var logPos = path.join(common.home, '.pm2/logs/totoro-server-out.log');
      var tail = new (require('tail').Tail)(logPos)
