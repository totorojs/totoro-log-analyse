...
getLog.get = function(cfg, cb) {
    var logResult = JSON.parse(fs.readFileSync('log.json'))
...
function checkOrderSucc(orderInfo) {
    var browsers = orderInfo.browsers
    var succ = true
    Object.keys(browsers).forEach(function(laborId) {
        var bInfo = browsers[laborId]
        if (bInfo.error) {
            succ = false
        }
    })
