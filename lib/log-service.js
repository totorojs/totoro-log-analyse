...
exports.getErrOrderList = function(cfg, callback) {
    var errorList = []
    getLog.get({}, function(orders) {
        orders.forEach(function(order) {
            var overview = getOrderOverview(order)
            if (!overview.succ) {
                errorList.push(overview)
            }
        })
    })
}
...
function getOrderOverview(order) {
    ...
    if (!order.totalTime) {
        ...
        bIds.forEach(function(bId) {
            ...
            var ua = testResult.ua
            var bInfo = ua.name + ' ' + ua.version

            if (testResult.error) {
                errMsg.push(bInfo + testResult.error.message)
                succ = false
            }

            if (testResult.failure) {
                errMsg.push(bInfo + ' test failure')
                succ = false
            }
...
function getDateById(id) {
    return moment(new Date(parseInt((id+'').slice(0,8), 16)*1000)).format('YYYY-MM-DD hh:mm:ss')
}
