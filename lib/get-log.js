...
function getSingleStatsCode(labor) {
  if (labor) {
    if (labor.terminated) {
      // NOTE
      // test interrupted abnormally for several times
      // labor.stats.error(before v0.6) -> labor.terminated
    } else if (labor.stats) {
      if (labor.stats.error) {
        // NOTE
        // Syntax error caught by window.onerror
        // labor.stats.error(before v0.6) -> labor.errors
        // no longer judge if a test succeed by this
