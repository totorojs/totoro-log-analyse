...
function getRecord(p, isWatch) {
  if (!(this instanceof getRecord)) return new getRecord(p, isWatch)
  this.p = p
  ...
  if (isWatch) {
    var watchObj = this.watchObj = getIncreasedRawRecord(p)
...
function getIncreasedRawRecord(p) {
  if (!(this instanceof getIncreasedRawRecord))
    return new getIncreasedRawRecord(p)
