...
function getOrderOverview(order) {
    ...
    if (order.errorList.length > 0) {
        ...
        bIds.forEach(function(bId) {
            ...
            var errInfo = {}
            errInfo.ua = ua.name + ' ' + ua.version
            var errors = errInfo.errors = []

            if (testResult.failures.length) {
                testResult.failures.forEach(function(msg) {
                    errors.push(msg.parent + ' > ' +  msg.title + ' > ' + msg.message)
                })
            } else if (testResult.duration < 0) {
                errors.push(' Illegal duration')
            } else if (stats.error) {
                var err = stats.error
                var url = err.url
                ...
                if (url) {
                    ...
                    errors.push(err.message + ' occurred at ' +  src + ' [' + err.line + ']')
                } else {
                    errors.push(err.message)
                }
            }
            if (errInfo.errors.length > 0) {
                errMsg.push(errInfo)
            }
        })
    }
